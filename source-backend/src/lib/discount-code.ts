/**
 * Pure discount math. Rules come from DB (`discount_codes`, including `apply_without_code` for auto-apply) via `resolve-discount.ts`.
 * Mirror calculation in `web/src/lib/discount-code.ts` for types / client display.
 *
 * Stacking: multiple codes (comma / semicolon / newline separated) apply in order; each step is
 * computed on the **original** base fee (e.g. two 10% on HKD 100 → −10 + −10 = −20, not compound).
 */
...
export type DiscountBreakdown = {
  ...
  finalFeeHkd: number;
  appliedLabel: string;
  /** Present when 2+ codes stacked — optional UI detail */
...
/**
 * Split user input into unique normalized codes (order preserved). Empty → [].
 */
