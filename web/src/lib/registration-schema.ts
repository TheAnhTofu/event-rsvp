import { z } from "zod";
import { committeeMeetingValues } from "@/lib/committee-meetings";

export { committeeMeetingValues } from "@/lib/committee-meetings";
...
/**
 * NFC normalization, strip zero-width chars, full-width space → ASCII space.
 * Improves validation and server parity for IME / Traditional Chinese input.
 */
...
/** Step 1 (Figma): Do you have any dietary preference? — Yes / No */
export const dietaryYesNoValues = ["yes", "no"] as const;

/** Step 2 (Figma): vegan / vegetarian / halal / gluten free / other (+ industry still uses halal). */
...
/** Lunch day(s) selected — show dietary step; "none" or unset skips dietary. */
...
/** Legacy + form display: infer Yes/No before `dietaryYesNo` existed. */
...
function migrateRegistrationDietaryPayload(raw: unknown): unknown {
...
export type RegistrationFormValues = {
  ...
  /** Members / IAIS Secretariat / AMF (Figma node 2:824). */
  memberDelegateRole?: (typeof memberDelegateRoleValues)[number];
  /** Supervisory jurisdiction (ISO country code from same list as Country). */
  jurisdiction?: string;
  ...
  /** Step 1: Yes = user has a preference; No = none. */
  dietaryYesNo?: (typeof dietaryYesNoValues)[number];
  /** Step 2: only when `dietaryYesNo === "yes"`. */
  dietary?: (typeof dietaryChoiceValues)[number];
  /** Free text when `dietary === "other"`; empty otherwise. */
  ...
  cpdApply: "yes" | "no";
  /** Figma: cancellation / refund policy acknowledgement (members). */
  refundPolicyAck?: boolean;
  /** Optional: Hong Kong visa / invitation letter (members). */
...
function readFeeHkdFromEnv(key: string, fallback: number): number {
  if (typeof process === "undefined" || !process.env?.[key]) return fallback;
  const n = Number.parseFloat(String(process.env[key]));
  return Number.isFinite(n) && n >= 0 ? n : fallback;
}

/**
 * Base fees (HKD) per homepage audience.
 * Simple in_person / online tiers (no audience) are fixed in code. Audience tiers use `NEXT_PUBLIC_*`.
 * Keep defaults in sync with `source-backend/src/lib/registration-schema.ts` and `.env.example`.
 */
...
/** Whole HKD amounts for UI copy (e.g. 8500 → "8,500"). */
export function formatHkdWholeAmount(amountHkd: number): string {
  return new Intl.NumberFormat("en-HK", {
    minimumFractionDigits: 0,
    maximumFractionDigits: Number.isInteger(amountHkd) ? 0 : 2,
  }).format(amountHkd);
}

/** Figma `2:824` fee row: `Registration fee : HKD 8,500.00`. */
export function formatRegistrationFeeLeadLine(amountHkd: number): string {
  const formatted = new Intl.NumberFormat("en-HK", {
    minimumFractionDigits: 2,
    maximumFractionDigits: 2,
  }).format(amountHkd);
  return `Registration fee : HKD ${formatted}`;
}
